...
// GeoadminSearch SDK retry feature
...
class GeoadminSearchRetryFeature extends GeoadminSearchBaseFeature
{
    ...
    public function init(GeoadminSearchContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GeoadminSearchContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GeoadminSearchContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GeoadminSearchContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
