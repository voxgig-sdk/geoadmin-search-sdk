...
class GeoadminSearchMakeError
{
    public static function call(?GeoadminSearchContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GeoadminSearchContext([], null);
        }
        $op = $ctx->op ?? new GeoadminSearchOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GeoadminSearchResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GeoadminSearchError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GeoadminSearchError('', $msg, $ctx);
        ...
        if ($err instanceof GeoadminSearchError) {
