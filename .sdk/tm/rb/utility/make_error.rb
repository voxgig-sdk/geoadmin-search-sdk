...
module GeoadminSearchUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = GeoadminSearchContext.new({}, nil)
    end
    op = ctx.op || GeoadminSearchOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || GeoadminSearchResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(GeoadminSearchError) ? err.msg : err.to_s
    ...
    sdk_err = GeoadminSearchError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(GeoadminSearchError)
