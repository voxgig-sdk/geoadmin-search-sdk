...
module GeoadminSearchUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GeoadminSearchResponse.new({})
    ...
    if fetch_err
      ...
      response = GeoadminSearchResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GeoadminSearchResponse.new(fetched)
    else
