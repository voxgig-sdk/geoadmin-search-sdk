...
class SearchEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GEOADMIN_SEARCH_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_basic_setup($extra)
{
    ...
    $client = GeoadminSearchSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("GEOADMIN_SEARCH_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GEOADMIN_SEARCH_TEST_SEARCH_ENTID"]);
    ...
    if ($env["GEOADMIN_SEARCH_TEST_LIVE"] === "TRUE") {
        ...
        $client = new GeoadminSearchSDK(Helpers::to_map($merged_opts));
    }
