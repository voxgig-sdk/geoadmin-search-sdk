...
// GeoadminSearch SDK feature test
//
...
class FtCtrl extends GeoadminSearchControl
{
...
class FtClient
{
    ...
    private GeoadminSearchUtility $utility;

    public function __construct(GeoadminSearchUtility $utility, array $options)
    {
    ...
    public function get_utility(): GeoadminSearchUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GeoadminSearchContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GeoadminSearchContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GeoadminSearchContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GeoadminSearchConfig::make_feature($name);
    ...
    private static function buildUrl(GeoadminSearchSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GeoadminSearchError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GeoadminSearchResponse($fetched);
        }
    ...
    private function fail(GeoadminSearchContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GeoadminSearchContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
