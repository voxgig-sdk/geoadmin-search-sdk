...
[MCP](https://modelcontextprotocol.io) server exposing the GeoadminSearch SDK as
two agent tools — `geoadmin-search_list` and `geoadmin-search_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geoadmin-search-mcp)
make build

# 2. Provide credentials via the environment
export GEOADMIN_SEARCH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user geoadmin-search \
  -- /absolute/path/to/geoadmin-search-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./geoadmin-search-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// geoadmin-search_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// geoadmin-search_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geoadmin-search-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user geoadmin-search \
     -- "$PWD"/dist/*/geoadmin-search-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `geoadmin-search_list` and `geoadmin-search_load` tools now appear
   in new sessions. Ask the agent to *"list search using geoadmin-search"*
   and it calls `geoadmin-search_list` with `{"entity":"search"}`.
...
```sh
export GEOADMIN_SEARCH_APIKEY=sk_live_xxx            # API key
export GEOADMIN_SEARCH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `geoadmin-search_list` tool
...
### Call the `geoadmin-search_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `geoadmin-search_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `geoadmin-search_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GEOADMIN_SEARCH_APIKEY` | API key sent with every request. |
| `GEOADMIN_SEARCH_BASE` | Optional override of the API base URL. |
...
```sh
./geoadmin-search-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"geoadmin-search_load","arguments":{"entity":"search","query":{"id":1}}}}'
```
