...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GEOADMINSEARCH_TEST_LIVE ||
    'TRUE' === process.env.GEOADMINSEARCH_TEST_OVERRIDE
  ) {
  ...
  m.GEOADMINSEARCH_TEST_EXPLAIN = process.env.GEOADMINSEARCH_TEST_EXPLAIN || m.GEOADMINSEARCH_TEST_EXPLAIN
