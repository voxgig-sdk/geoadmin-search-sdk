...
const __1 = require("..");
const SDK_NAME = 'GeoadminSearchSDK';
...
const TEST_SEED = { "entity": { "search": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+GeoadminSearchSDK\b/.test(code) ||
        /\bGeoadminSearchSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+GeoadminSearchSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/GeoadminSearchSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
