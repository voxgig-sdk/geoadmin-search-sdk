# GeoadminSearch SDK Search entity
...
class SearchEntity
  ...
  def data_set(args)
    if args
      @_data = GeoadminSearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = GeoadminSearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Search, Hash] the loaded Search; raises GeoadminSearchError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GeoadminSearchHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
