...
require_relative 'GeoadminSearch_types'
...
class GeoadminSearchSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GeoadminSearchHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GeoadminSearchHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GeoadminSearchFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GeoadminSearchUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GeoadminSearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GeoadminSearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GeoadminSearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => GeoadminSearchError.new(
        "#{op}_allow",
        "GeoadminSearchSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue GeoadminSearchError => err
    ...
    if fetched.is_a?(Hash)
      status = GeoadminSearchHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GeoadminSearchError.new(
        "graphql_error", "GeoadminSearchSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GeoadminSearchSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
