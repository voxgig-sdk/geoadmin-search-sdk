# GeoadminSearch SDK test runner
...
class GeoadminSearchTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GeoadminSearchTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GeoadminSearchTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GeoadminSearchTestRunner.getenv("GEOADMIN_SEARCH_TEST_LIVE")
        override = GeoadminSearchTestRunner.getenv("GEOADMIN_SEARCH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GeoadminSearchTestRunner.getenv(key)
        ...
        explain = GeoadminSearchTestRunner.getenv("GEOADMIN_SEARCH_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GEOADMIN_SEARCH_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GeoadminSearchTestRunner._test_control is not None:
            return GeoadminSearchTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GeoadminSearchTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GeoadminSearchTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GeoadminSearchTestRunner.load_test_control()
...
def load_env_local():
    GeoadminSearchTestRunner.load_env_local()


def env_override(m):
    return GeoadminSearchTestRunner.env_override(m)


def entity_data(v):
    return GeoadminSearchTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GeoadminSearchTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GeoadminSearchTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GeoadminSearchTestRunner.load_test_control()


def live_client_options():
    return GeoadminSearchTestRunner.live_client_options()


def live_delay_ms():
    return GeoadminSearchTestRunner.live_delay_ms()
