...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GEOADMINSEARCH_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GEOADMINSEARCH_TEST_SEARCH_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"GEOADMINSEARCH_TEST_SEARCH_ENTID": idmap,
		"GEOADMINSEARCH_TEST_LIVE":      "FALSE",
		"GEOADMINSEARCH_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["GEOADMINSEARCH_TEST_SEARCH_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["GEOADMINSEARCH_TEST_LIVE"] == "TRUE" {
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["GEOADMINSEARCH_TEST_EXPLAIN"] == "TRUE",
		live:          live,
