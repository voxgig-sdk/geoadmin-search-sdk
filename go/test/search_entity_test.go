...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GEOADMIN_SEARCH_TEST_SEARCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GEOADMIN_SEARCH_TEST_SEARCH_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["GEOADMIN_SEARCH_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
			},
			extra,
		})
		client = sdk.NewGeoadminSearchSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["GEOADMIN_SEARCH_TEST_EXPLAIN"] == "TRUE",
		live:          live,
