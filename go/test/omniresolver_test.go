...
import (
	...
	omni "github.com/voxgig-sdk/geoadmin-search-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GeoadminSearchError); is {
				out := map[string]any{
					"name":    "GeoadminSearchError",
...
func omniCtx(arg any, client *sdk.GeoadminSearchSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GeoadminSearchSDK); is {
			client = live
