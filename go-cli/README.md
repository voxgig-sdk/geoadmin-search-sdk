...
boru-driven command-line client **and** interactive REPL for the GeoadminSearch
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geoadmin-search-cli)
make build

# 2. See usage (words, entities, env vars)
./geoadmin-search-cli --help

# 3. Provide credentials once, via the environment
export GEOADMIN_SEARCH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./geoadmin-search-cli load 1 search            # {id:1} shorthand
./geoadmin-search-cli load '{id:1}' search       # explicit match map

# 5. Override the API base URL for a single call
GEOADMIN_SEARCH_BASE=https://api.example.com ./geoadmin-search-cli load 1 search

# 6. No arguments -> interactive REPL
./geoadmin-search-cli
geoadmin-search> load 1 search
geoadmin-search> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geoadmin-search-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/geoadmin-search-cli load 1 search
   ```
...
```sh
./geoadmin-search-cli load 1 search          # scalar shorthand for {id:1}
./geoadmin-search-cli load '{id:1}' search     # explicit match map
```
...
./geoadmin-search-cli load 1 search
```
...
Run with no arguments to open a REPL (prompt `geoadmin-search>`). Each line is
evaluated as its own boru expression:

```text
$ ./geoadmin-search-cli
geoadmin-search> load 1 search
geoadmin-search> /help
geoadmin-search> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `search`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/geoadmin-search-cli`. |
...
(`./geoadmin-search-cli <expr>`) and interactively (the REPL), and expressions compose the
