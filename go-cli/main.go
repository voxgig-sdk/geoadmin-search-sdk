// Command GeoadminSearch-cli is a CLI + REPL for the GeoadminSearch SDK,
...
const prompt = "geoadmin-search"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GEOADMIN_SEARCH_APIKEY (api key), GEOADMIN_SEARCH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GEOADMIN_SEARCH_APIKEY carries the API key and
	// GEOADMIN_SEARCH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GEOADMIN_SEARCH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GEOADMIN_SEARCH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGeoadminSearchSDK(opts)
